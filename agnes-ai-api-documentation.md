...
同一用户可以同时拥有多种访问类型（如同时持有默认、企业、Token Plan 三种密钥），不同类型的密钥使用不同的限制池；相同类型的多个密钥**共享**同一个限制池，不会叠加额度。
...
> 分辨率越高 RPM 越低；Token Plan 仅在 1K/2K 档位享有更高的图片生成能力，3K/4K 档位各用户类型限制基本一致。
...
### 10.5 Token Plan 订阅配额

| Plan | 文本 `agnes-2.5-flash` | 图片 `agnes-image-2.1-flash` | 视频 `agnes-video-v2.0` / `agnes-video-2.5-flash` |
|------|------------------------|----------------------------|--------------------------------------------------|
...
- 文本配额按**请求次数**计数；图片按**生成图片张数**计数；视频按**生成视频时长（秒）**计数
- **RPM 限制与订阅配额同时生效**：即使 Token Plan Pro 文本支持 1000 RPM，仍受每 5 小时 30,000 次 / 每周 300,000 次的配额约束
- 创建多个相同类型的密钥不会叠加 RPM 或配额
- 配额或 RPM 超标会触发 429 限流，建议在客户端实现队列与退避重试
